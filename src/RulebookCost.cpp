...
RulebookCost::RulebookCost(double v) : value(v) {
    if (!rulebook) {
...
void RulebookCost::setRuleCost(size_t rule_index, double cost) {
    cost_vector[rule_index]->setValue(cost);
}

double RulebookCost::getRuleCost(size_t index) const {
...
void RulebookCost::setRulebook(const Rulebook &rb) {
    rulebook = std::make_unique<Rulebook>(rb);
}

RulebookCost RulebookCost::getZero() { 
    return RulebookCost(); 
}

std::ostream &operator<<(std::ostream &os, const RulebookCost &c) {
    os << "[";
...
RulebookCost RulebookCost::operator+(const RulebookCost &other) const {
    if (cost_vector.size() != other.cost_vector.size()) {
        std::cerr << "This cost: " << cost_vector.size()
                  << " other: " << other.cost_vector.size() << std::endl;
        throw std::invalid_argument("Cost vectors must be of the same size");
    }
...
bool RulebookCost::operator<=(const RulebookCost &other) const {
    ...
    for (auto it = rulebook->begin(); it != rulebook->end(); ++it) {
        for (auto rule_index : *it) {
            ...
            if (rule_cost < other_cost) {
                ...
                overridden_rules.insert(successors.begin(), successors.end());
            }
        }
    }
    return true;
