...
void addResult(std::unique_ptr<Scenario> scenario, std::ofstream &outdata) {
    ...
    const auto res = scenario->getOptimalPlans();
    auto stop = high_resolution_clock::now();
    auto duration = duration_cast<milliseconds>(stop - start);
    //auto duration = duration_cast<microseconds>(stop - start);

    outdata << "{\"psize\": " << scenario->getGraphSize() << ", ";
    outdata << "\"rsize\": " << scenario->getNumRules() << ", ";
            ...
            if (!args.change_rule_size) {
                for (size_t num_x = 5; num_x < max_grid; ++num_x) {
                    for (size_t num_y = 5; num_y < max_grid; ++ num_y) {
                        scenario = std::make_unique<ScenarioRandom>(num_x, num_y, 5);
                        addResult(std::move(scenario), outdata);
                        if (i + 1 < args.num_experiments || num_x + 1 < max_grid || num_y + 1 < max_grid)
                            outdata << ", ";
                    }
                }
            }
                ...
                for (size_t num_rules = 1; num_rules <= max_rules; ++num_rules) {
                    scenario = std::make_unique<ScenarioRandom>(10, 10, num_rules);
                    addResult(std::move(scenario), outdata);
                    if (i + 1 < args.num_experiments || num_rules + 1 <= max_rules)
                        outdata << ", ";
                }
            }
        } else {
            scenario = std::make_unique<ScenarioAvoidance>();
            addResult(std::move(scenario), outdata);
