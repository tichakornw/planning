...
ScenarioRandom::ScenarioRandom(size_t num_x, size_t num_y, size_t num_rules)
    : num_x(num_x), num_y(num_y), num_rules(num_rules) {
    setRandomInitAndGoal();
    setup();
}

void ScenarioRandom::setRandomInitAndGoal() {
    init_vid = getRandomInt(0, (num_x * num_y) - 1);
    goal_vid = getRandomInt(0, (num_x * num_y) - 1);
}

size_t ScenarioRandom::getRandomInt(size_t min_value, size_t max_value) {
...
double ScenarioRandom::getRandomDouble(double min_value, double max_value) {
...
std::vector<std::unordered_set<size_t>> ScenarioRandom::getRandomSubsets(int n) {
    std::vector<int> S(n);
...
RulebookCost ScenarioRandom::getRandomCost() {
    RulebookCost cost;
...
void ScenarioRandom::defineRulebook() {
...
void ScenarioRandom::buildGraph() {
