...
ScenarioAvoidance::ScenarioAvoidance(size_t refinement_option)
...
void ScenarioAvoidance::defineRulebook() {
...
void ScenarioAvoidance::buildGraph() {
...
DiscreteState2D ScenarioAvoidance::getStateAt(size_t row, size_t col) {
    return DiscreteState2D(col + 1, row + 1);
}

bool ScenarioAvoidance::isInObstacle(size_t row, size_t col, size_t obstacle_x_min,
...
bool ScenarioAvoidance::isNearObstacle(size_t row, size_t col, size_t obstacle_x_min,
                                       size_t obstacle_x_max) {
    if (row > 1)
        return false;
    return (col >= obstacle_x_min && col <= obstacle_x_max);
...
bool ScenarioAvoidance::isNotInLane(size_t row) { 
    return (row > 1); 
}

RulebookCost ScenarioAvoidance::getCost(size_t prev_row, size_t prev_col, size_t next_row,
