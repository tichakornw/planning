...
// From navigation.cpp
void testNavigation();
std::pair<RulebookCost, double> runNavigationPlanning(
    const ScenarioNavigation &scenario,
    size_t iterations,
    size_t retry,
...
std::vector<std::pair<RulebookCost, double>> runPlanner(const ScenarioNavigation &scenario, size_t num_runs, size_t iterations) {
    ...
    for (size_t i = 0; i < num_runs; ++i) {
        auto [total_cost, elapsed_ms] = runNavigationPlanning(scenario, iterations, 10);
...
void toJsonStreamStats(std::ostream &os, const std::vector<std::pair<RulebookCost, double>>& results, size_t iterations, bool is_last) {
    ...
    for (size_t i = 0; i < results.size(); ++i) {
        ...
        os << "      \"path_cost\": " << cost << "\n";
        os << "    }";
        if (!is_last || i + 1 < results.size()) os << ",";
        os << "\n";
    }
}
...
void collectStats() {
    const std::vector<size_t> all_iterations =
        {100,   167,   278,   464,   774,  1292,  2154,  3594,  5995, 10000};
